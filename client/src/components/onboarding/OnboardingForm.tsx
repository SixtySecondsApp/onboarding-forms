...
import { type OnboardingForm as FormType } from '@shared/schema';
...
interface FormState {
  ...
  salesData: {
    salesType: 'inbound' | 'outbound' | 'hybrid';
    meetingsBeforeProposal: string;
    proposalConversionRate: string;
...
type FormFieldName = keyof BusinessDetails | keyof FormState['campaign'] | keyof FormState['audience'] | keyof FormState['salesData'] | keyof FormState['brandAssets'];
...
interface FormField {
  label: string;
  name: keyof FormState[keyof FormState];
  icon: React.ForwardRefExoticComponent<any>;
  placeholder: string;
  type?: string;
  options?: { value: string; label: string; }[];
...
interface FormFields {
  ...
  audienceFields: FormField[];
  salesFields: FormField[];
}

// Valid sales types
type SalesType = 'inbound' | 'outbound' | 'hybrid';
...
export function OnboardingForm({ formId, sectionId }: Props) {
  ...
  // Steps configuration with getStepStatus
  const steps = [
  ...
  const validateField = (name: string, value: string): string => {
    ...
    switch (name) {
      ...
      case 'companySize':
        ...
        break;
      case 'salesType':
        if (!value) error = 'Please select a sales type';
        break;
    }
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (name === 'phone') {
      ...
      setBusinessDetails(prev => ({ ...prev, [name]: formattedValue }));
    } else {
      if (name in campaign) {
        setCampaign(prev => ({ ...prev, [name]: value }));
      } else if (name in audience) {
        setAudience(prev => ({ ...prev, [name]: value }));
      } else if (name in salesData) {
  ...
  const handleBlur = (e: React.FocusEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    let value = '';
    if (name in businessDetails) {
      value = businessDetails[name as keyof typeof businessDetails];
    } else if (name in campaign) {
      value = campaign[name as keyof typeof campaign];
    } else if (name in audience) {
      value = audience[name as keyof typeof audience];
    } else if (name in salesData) {
      value = salesData[name as keyof typeof salesData];
    }
  ...
  const handleComplete = async () => {
    ...
    Object.keys(businessDetails).forEach(field => {
      ...
      const error = validateField(field, businessDetails[field as keyof BusinessDetails]);
      if (error) {
        hasErrors = true;
    ...
    Object.keys(campaign).forEach(field => {
      if (field !== 'keyMessages' && field !== 'callToAction') { // Optional fields
        newTouched[field] = true;
        const error = validateField(field, campaign[field as keyof typeof campaign]);
        if (error) {
          hasErrors = true;
          newErrors[field] = error;
        }
    ...
    Object.keys(audience).forEach(field => {
      newTouched[field] = true;
      const error = validateField(field, audience[field as keyof typeof audience]);
      if (error) {
        hasErrors = true;
        newErrors[field] = error;
      }
  ...
  interface FormField {
    label: string;
    name: keyof BusinessDetails | "campaignName" | "objective" | "jobTitles" | "industries" | "companySize" | "salesType";
    icon: any;
    placeholder: string;
    type?: string;
    options?: { value: string; label: string; }[];
  ...
  const validateField = (name: keyof BusinessDetails | "campaignName" | "objective" | "jobTitles" | "industries" | "companySize" | "salesType", value: string) => {
    let error = '';
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    // Validate on change
    const error = validateField(name as keyof BusinessDetails | "campaignName" | "objective" | "jobTitles" | "industries" | "companySize" | "salesType", name === 'phone' ? formatPhoneNumber(value) : value);
    setErrors(prev => ({
      ...prev,
      [name]: error
    }));
  ...
  const handleBlur = (e: React.FocusEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    const error = validateField(name as keyof BusinessDetails | "campaignName" | "objective" | "jobTitles" | "industries" | "companySize" | "salesType", value);
    setErrors(prev => ({
      ...prev,
      [name]: error
    }));
        ...
        <FormSection>
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <textarea
                placeholder="Enter target locations (e.g., North America, Europe, Global)"
                className="w-full h-24 bg-gray800/50 border border-gray-700 rounded-lg p-3 text-gray-200"
                value={audience.locations}
                onChange={handleAudienceChange}
              />
            </div>
